...
bool CCreateur::CreerZone()
{
	...
	for (int i=pSalle; i<pSalle+nSalles; i++)
	{
		...
		do
		{
			...
			//On v�rifie que le Salle n'existe pas d�j�
			ok=true;
...
#define NMAXESSAIS 1000
	...
	while (nEssais<NMAXESSAIS && (NombrePerc || i<nPortes))
	{
		...
		do
		{
			...
			//On v�rifie qu'aucun Salle n'appartient � la Porte
			...
			//ON v�rifie qu'on croise pas une autre Porte			
